라운드 10 — 시각화 도구 (옵션 C). 톨로런스 폐기 여부는 시각화 결과 보고 라운드 11에서 결정.
배경 및 목적
라운드 9까지 도면1~5 기둥 회귀 14/16 PASS (도면5 C1·C2는 톨로런스 ±1로 통과). 카운트 결과는 숫자로만 보고 있어서 어떤 텍스트가 본체로 인정됐는지, 어떤 게 일람표/규격/필터로 제외됐는지 시각적으로 확인하기 어렵다. 라운드 11 (톨로런스 정확화 + 일람표 검출 정밀화) 의사결정의 근거 데이터 확보가 이번 라운드의 목적.
...
pythondef classify_drawing_texts(drawing: str) -> list[dict]:
    ...
    """
category 6분류:
라벨의미시각화 색counted본체로 카운트됨 (final 에 반영)녹색filtered_heightmin_height 필터로 제외회색filtered_spec규격 안내 (exclude_with_spec=True 로 제외)노란색filtered_table일람표 영역 안에 있어서 제외빨간색not_whitelist화이트리스트 매칭 실패 (부호 아님)표시 안 함slash_combo_body슬래시 결합 본체 (도면3·5의 C1/P1 등 — counted 의 하위 분류)진한 녹색
구현 방식:
...
A 매칭 실패 → not_whitelist
height 컷에 걸림 → filtered_height
B 매칭 실패 (A 성공) → filtered_spec (= 규격 결합 또는 슬래시 결합 중 후자)
...
categorymarkercolorsize의미countedcircle-opengreen14본체 카운트slash_combo_bodycircledarkgreen14슬래시 결합 본체filtered_heightxgray10height 필터로 제외filtered_specxgold10규격 안내 제외filtered_tablexred12일람표 영역 제외not_whitelist(표시 안 함)--부호 아님
레전드는 부호별이 아니라 분류별. hover 시 텍스트·좌표·분류·매칭 부호 표시.
2-3. 일람표 영역 bbox 사각형
compute_drawing 의 regions 결과 → 빨간 점선 사각형으로 표시. 사각형 안에 영역 인덱스와 부호 카운트 표시 (예: "Region 0: C1(1), C2(1), C3(1), C4(1)").
2-4. 정답 비교 박스
플롯 한 모서리에 작은 텍스트 박스로 도면별 카운트 결과 표시:
...
C3         8       8       0
C4         6       6       0
compute_drawing 결과 그대로 사용.
2-5. 출력
outputs/visualize/도면1_detection.html
outputs/visualize/도면2_detection.html
outputs/visualize/도면3_detection.html
outputs/visualize/도면4_detection.html
outputs/visualize/도면5_detection.html
각 파일은 standalone (CDN plotly.js 사용해서 브라우저에서 바로 열림). 외부 의존성 없음.
작업 3 — CLI 진입점
python poc_v2/tests/visualize_detection.py 실행 시:

인자 없으면 5개 도면 전부 처리
인자 1개 (예: python visualize_detection.py 도면3) 면 해당 도면만 처리
각 도면 처리 후 콘솔에 "outputs/visualize/도면N_detection.html 생성" 로그
...
A. 라운드 10 목적 — 라운드 11 의사결정 근거 확보
B. 신설 파일 목록 — classify_text.py, visualize_detection.py
C. 출력 5개 HTML 위치 — outputs/visualize/
D. 도면별 시각화 결과 요약 — 사용자가 눈으로 확인할 포인트 (아래 표 형식):

도면본체 마커 수일람표 영역규격 제외height 컷시각적 검증 포인트도면1????일람표가 도면에 있는지 눈으로 확인 (코드는 0곳 검출)도면2????SC1·SC2 누락 위치 시각적 확인도면3????C1/P1 슬래시 결합 본체 위치도면4????SB1 좌표 중복 위치도면5????C1·C2 +1 오차의 원인 위치 (라운드 11 핵심)
...
도면1 일람표가 물리적으로 존재하는지 (이번 사용자 질문)
도면5 C1·C2 의 +1 오차가 어디서 오는지 (일람표 검출 한계 vs 분류 알고리즘 버그)
도면2 SC1·SC2 분리 TEXT 위치 (라운드 4 갈래 C 진단 시각화)
도면4 SB1 좌표 중복 위치 (라운드 5 보류 항목 진단)
...
outputs/visualize/도면N_detection.html × 5 — 신규
회귀 결과 (14/2 유지)
콘솔 로그: 5개 도면 처리 결과
...
외부 라이브러리 추가 금지: ezdxf + plotly + 표준 라이브러리만 (plotly 는 poc_v3 에서 이미 도입됨)
